...
//            var.toArray(new EMMessage[var.size()]);
//            conversation.markAllMessagesAsRead();
    ...
    protected EaseChatRow createChatRow(Context context, EMMessage message, int position) {
        ...
        if (customRowProvider != null) {
            EaseChatRow customeaseChatRow = customRowProvider.getCustomChatRow(message, position, this);
            if (customeaseChatRow != null)
                return customeaseChatRow;
        }
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        EMMessage message = getItem(position);
        if (convertView == null) {
            String money1 = message.getStringAttribute(CustomConstant.ESSAGE_ATTRIBUTE_RED_PACKET_MONEY, "");
